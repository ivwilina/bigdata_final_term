...
from pyspark.sql.functions import  when, desc, year, sum, lit, when, min, avg, max, col, split, explode, count, lower
...
print('### Câu 3.4 ###')
latest = data.orderBy(desc('Time')).collect()[0][2]
...
print('### Câu 3.6 ###')
ars_point = ars_win*3 + data.filter(col('HomeTeam')=='Arsenal').filter(col('FTR')=='D').count()*1 + data.filter(col('AwayTeam')=='Arsenal').filter(col('FTR')=='D').count()*1
print(f'Số điểm mà Arsenal dành được là {ars_point}')

print('### Câu 3.7 ###')
def pointCount(x):
    point = data.filter(col('HomeTeam')==x).filter(col('FTR')=='D').count()*1 + data.filter(col('AwayTeam')==x).filter(col('FTR')=='D').count()*1 + data.filter(col('HomeTeam')==x).filter(col('FTR')=='H').count()*3 + data.filter(col('AwayTeam')==x).filter(col('FTR')=='A').count()*3
    return point
...
print('### Câu 3.10 ###')
data.withColumn('3.10',when(col('Time').between("00:00","12:00"),'Time 1').when(col('Time').between("12:00","18:00"),'Time 2').when(col('Time').between("18:00","24:00"),'Time 3')).show()